...
/**
 * Provides SMTP mail support
 **/
public class CustomMailProvider extends AbstractMailProvider {

    private static final Logger LOGGER = LoggerFactory.getLogger(CustomMailProvider.class);
    ...
    public void send(MailMessage message) {
        ...
        try {
            ...
            if (message instanceof CustomMailMessage) {
                CustomMailMessage customMailMessage = (CustomMailMessage) message;
                for (Attachment attachment : customMailMessage.getAttachments()) {
                    try {
                        multiPartContent.addBodyPart(buildAttachment(attachment));
                    } catch (Exception e) {
                        LOGGER.warn("Could not add attachment", e);
                    }
                }
            }
            ...
            LOGGER.debug("Sent email to [{}] with subject [{}].", message.getTo(), message.getSubject());
    ...
    private MimeBodyPart buildAttachment(Attachment fileAttachment) throws MessagingException, IOException {
        MimeBodyPart attachment = new MimeBodyPart(fileAttachment.getHeaders(), fileAttachment.getBytes());
